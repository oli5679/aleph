...
//!
//! Polyglot is the standard opening book format used by chess engines.
//! Each entry is 16 bytes: key (u64), move (u16), weight (u16), learn (u32).
//! The file is sorted by key for efficient binary search.
...
impl OpeningBook {
    ...
    pub fn load<P: AsRef<Path>>(path: P) -> io::Result<Self> {
        ...
        if data.len() % 16 != 0 {
            return Err(io::Error::new(
                ...
                "Invalid polyglot book: size not multiple of 16",
            ));
        }
    ...
    /// Probe the book for a position. Returns the best move if found.
    /// Uses weighted random selection when multiple moves are available.
    pub fn probe(&self, pos: &Position) -> Option<Move> {
        let key = polyglot_hash(pos);
        let entries = self.find_entries(key);

        if entries.is_empty() {
            return None;
        }

        // Select move based on weights (deterministic: pick highest weight)
        // For variety, could use weighted random selection
        let best = entries.iter().max_by_key(|e| e.weight)?;
        decode_polyglot_move(pos, best.raw_move)
    }

    /// Probe and return all book moves with their weights.
    pub fn probe_all(&self, pos: &Position) -> Vec<(Move, u16)> {
        let key = polyglot_hash(pos);
        self.find_entries(key)
            .iter()
            .filter_map(|e| decode_polyglot_move(pos, e.raw_move).map(|m| (m, e.weight)))
            .collect()
    }

    /// Find all entries matching the given key using binary search.
...
/// Decode a Polyglot move encoding to our Move type.
fn decode_polyglot_move(pos: &Position, raw: u16) -> Option<Move> {
    // Polyglot move encoding:
    // bits 0-5: to file (0-7) and to rank (0-7) combined as to_sq
    // bits 6-11: from file (0-7) and from rank (0-7) combined as from_sq
    ...
    // Handle castling: Polyglot encodes king move to rook square
    if let Some((_, Piece::King)) = pos.piece_at(from) {
        // Kingside castling
        if from_file == 4 && to_file == 7 {
            to = Square::new(6, from_rank); // g1 or g8
        }
        // Queenside castling
        if from_file == 4 && to_file == 0 {
            to = Square::new(2, from_rank); // c1 or c8
        }
    }
    ...
    for mv in moves.iter() {
        ...
        if promo > 0 {
            ...
            let expected_piece = match promo {
                1 => Piece::Knight,
                2 => Piece::Bishop,
                3 => Piece::Rook,
                4 => Piece::Queen,
            ...
            if mv.promotion_piece() != expected_piece {
                continue;
            }
        } else if mv.is_promotion() {
            // Default to queen promotion if no promo specified
            if mv.promotion_piece() != Piece::Queen {
                continue;
            }
        }
...
/// This must match the standard Polyglot hash computation.
fn polyglot_hash(pos: &Position) -> u64 {
    ...
    for sq_idx in 0..64usize {
        let square = Square(sq_idx as u8);
        if let Some((color, piece)) = pos.piece_at(square) {
            let piece_idx = polyglot_piece_index(color, piece);
            let key_idx = 64 * piece_idx + sq_idx;
            hash ^= POLYGLOT_KEYS[key_idx];
        }
    }
    ...
    // En passant file (only if there's actually an en passant capture possible)
    if let Some(ep_sq) = pos.ep_square() {
        // Check if en passant capture is actually possible
        if ep_capture_possible(pos, ep_sq) {
            let file = ep_sq.file();
            hash ^= POLYGLOT_KEYS[772 + file as usize];
        }
    }
...
/// Check if en passant capture is actually possible (a pawn can capture).
fn ep_capture_possible(pos: &Position, ep_sq: Square) -> bool {
    use crate::types::Color;

    let file = ep_sq.file();
    let rank = ep_sq.rank();

    // The capturing pawn must be on an adjacent file and correct rank
    let pawn_rank = if pos.side_to_move() == Color::White { 4 } else { 3 };

    if rank != if pos.side_to_move() == Color::White { 5 } else { 2 } {
        return false;
    }

    // Check adjacent files for our pawns
    let our_pawns = pos.pieces(pos.side_to_move(), Piece::Pawn);

    if file > 0 {
        let left_sq = Square::new(file - 1, pawn_rank);
        if our_pawns.contains(left_sq) {
            return true;
        }
    }
    if file < 7 {
        let right_sq = Square::new(file + 1, pawn_rank);
        if our_pawns.contains(right_sq) {
            return true;
        }
    }
...
/// Map our piece types to Polyglot piece indices.
fn polyglot_piece_index(color: crate::types::Color, piece: Piece) -> usize {
    use crate::types::Color;
    ...
    base + if color == Color::Black { 0 } else { 1 }
}

// Polyglot Zobrist keys (standard values)
#[rustfmt::skip]
...
mod tests {
    ...
    fn test_entry_parsing() {
        // Create a minimal test with known data
        let book = OpeningBook { data: Vec::new() };
